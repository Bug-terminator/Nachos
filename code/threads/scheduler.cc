...
//     //如果该线程的优先级高于正在运行的线程，直接抢占
//     if (thread->getPrio() < currentThread->getPrio())   
//     {
//         cout << "thread " << currentThread->getTID() << " is deprived by thread " << thread->getTID() << " ,previous thread's priority = " << currentThread->getPrio() << ", current thread's priority = " << thread->getPrio() << endl;
//         currentThread->Yield();
//     }
// }
